...
import markDisplay from "../../public/media/mark.svg";
import markSmallCut from "../../public/media/mark-small.svg";
    ...
    <div className="mx-auto max-w-content px-4 pb-4 pt-12 sm:px-6 sm:pt-20">
      ...
      <section className="mt-16">
        ...
        <div className="grid gap-8 rounded-lg border border-line bg-surface p-6 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-10">
          <div className="flex shrink-0 items-end gap-5 self-start">
            <figure>
              <Image src={markDisplay} alt="" width={96} height={96} />
              <figcaption className="mono mt-2 text-[10px] text-dim">
                display
              </figcaption>
            </figure>
            <figure>
              {/* Shown enlarged with pixelated scaling, so the grid it is
                  actually drawn on is visible. */}
              <Image
                src={markSmallCut}
                alt=""
                width={64}
          ...
          <div>
            <h3 className="title text-[17px]">A P set on the shelf</h3>
            <p className="prose-body mt-2 text-[15px]">
              The mark is a P built from the same squircle tiles as the product
              shelf on the home page. Every filled tile is something shipped.
              One slot is open, in blue, for the next one, and it sits where the
              bowl closes back to the stem: the last piece.
            </p>
            <p className="prose-body mt-3 text-[15px]">
              It is the third attempt. The first was a damped spring plotted
              from{" "}
              <code className="mono text-[13px] text-fg">
                y(t) = A·e^(−ζωt)·cos(ω_d·t)
              </code>
              , which had an idea and could not be read: a stray &ldquo;w&rdquo;
              at 96px, lint at 16. The second was a plain P, which could be read
              and had no idea at all. A mark has to do both before it does
              anything else.
            </p>
            <p className="prose-body mt-3 text-[15px]">
              A grid mark cannot simply be scaled down, so it is drawn twice.
              The display cut uses squircle tiles and draws the open slot as an
              outline. The small cut is drawn on a real 16×16 pixel grid, 2px
              tiles and 1px gaps on whole pixels, so every gap stays open in a
              browser tab. A 2px square cannot carry an outline, so there the
              open slot becomes a single blue pixel.
            </p>
            <p className="prose-body mt-3 text-[15px]">
              Both are generated by{" "}
              <code className="mono text-[13px] text-fg">scripts/mark.mjs</code>
              , not drawn, so the letter is a five-line grid anyone can read in
              the source.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-line bg-surface p-6">
          ...
          <ul className="mt-5 flex flex-wrap gap-2">
            {[
              ...
              { href: "/media/mark.svg", label: "Mark, display", size: "SVG" },
              { href: "/media/mark-1024.png", label: "Mark", size: "1024 PNG" },
              {
                href: "/media/mark-small.svg",
                label: "Mark, 16px grid",
                size: "SVG",
              },
            ].map((a) => (
              <li key={a.href}>
